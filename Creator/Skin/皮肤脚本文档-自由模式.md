...
```lua
{
    ...
    segments = list -- 片段数据
    sample = "aaa.ogg" -- Note附带的key音文件名,6.3.32添加
}
...
-- 当Play结束时触发，如果脚本申明此函数，则Play结束时不会自动跳转结算界面
-- 可选，6.5.32起可用
function OnFinish()
end
...
| lua函数名 | 定义 | 备注 |
|- |- |- |
|**6.6.43** DoRequest(tag, url, method, data)|发起异步HTTP请求，参数分别是：<br>---<br>tag: string, 请求标记，会在回调里原样返回<br>url: string, 完整请求地址<br>method: string, 请求方法，支持 GET/POST（不区分大小写），为空时按GET处理<br>data: string, 请求参数字符串，支持 JSON Object 字符串或 query string（如 `a=1&b=2`）|仅自由模式运行时可用。Composer里不生效。|

回调说明：
- 请求完成后通过 `OnResponse(tag, content)` 回调（无论成功或失败）。
