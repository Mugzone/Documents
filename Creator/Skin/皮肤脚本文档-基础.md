...
| [+ 6.3.22 +]LoadChart(name): res | 加载一个位于谱面目录下的音频资源，返回资源 id:res                        | 支持 mp3,ogg,wav 格式 |
...
| [+ 6.5.2 +] PlayMeta(string): number          | 获取游戏设定<br>----<br>支持值: speed,mod,judge | |
...
| [+ 6.6.12 +]CustomInfo():string   | 读取通过ReportCustomInfo写入的自定义内容 | | 
| [+ 6.6.22 +]FileHash(name): string | 获取文件MD5值 | |
| [+ 6.6.22 +]ReadChartCustom(name): string | 获取谱面中保存的自定义值 | |
